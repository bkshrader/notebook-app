...
Given our constraints — a11y-first (WCAG 2.1 AA floor), ADHD-stable UI, Helios-token theming with a `DESIGN.md` visual contract, and an existing Storybook + `storybook-mcp` workflow — the survey points to a clear target stack:

- **Accessibility — enforce it, don't just panel it (highest-value adoption).** Follow **Grafana's pattern**: `a11y: { test: 'error' }` in `preview.ts` + `@storybook/test-runner` in CI, run across **every Helios token mode** (light / dark / high-contrast). This directly serves the charter floor. The `@storybook/addon-a11y` panel alone — the most common pattern in the cohort — would _not_ satisfy our constraint.
...
- **Visual — Chromatic with per-mode `chromatic.modes`** (Carbon's pattern) to guard the `DESIGN.md` contract across themes, gated to changed stories for cost. The free OSS tier fits a public AGPL repo.
- **Test-runner** is the engine that makes the above run in CI — adopt alongside.
- **Skip** Storyshots (deprecated, zero adoption) and `@storybook/addon-coverage` (zero adoption). If we want story-driven coverage later, use `@storybook/addon-vitest` + `@vitest/coverage-v8`, not the coverage addon. CI already gates the seven checks in [`CLAUDE.md`](../../CLAUDE.md); Storybook a11y/interaction tests would be an additive gate, not a replacement.

When this moves from survey to plan, it warrants an ADR under `docs/features/accessibility/adrs/` (the a11y enforcement choice trades CI time against the charter floor and is non-obvious enough to record).
