...
> **Package-manager note.** This ADR and its incident chronology were written while the project used **npm**, and the incident records below intentionally retain their original `npm`/`npx`/`npm ci` wording as a faithful account of what happened at the time. Later the same day the project migrated to **pnpm** (see [package-manager.md](package-manager.md)). The hooks' _current_ contract — summarized in "Concrete hook responsibilities" below — uses `pnpm` (`pnpm exec`, `pnpm install`, `pnpm audit`); where an incident narrative says `npm`, read it as "the package manager at the time." The four invariants are package-manager-agnostic and unchanged.
...
- **`husky-bootstrap.sh` (SessionStart + CwdChanged)** — bootstrap husky in EXACTLY the environment the triggering event points at. The target directory is read from the hook's stdin payload — `new_cwd` for CwdChanged, `cwd` for SessionStart — with `CLAUDE_PROJECT_DIR` as a fallback for manual invocation; one stdin-driven script serves both events. No cross-environment reach: it does not resolve or bootstrap the main repo from a worktree. Repairs any worktree-scope `core.hooksPath` override (`git config --worktree --unset core.hooksPath`); skips loudly on a ghost worktree (`[ ! -e .git ]`); requires a local husky install (`[ -d node_modules/husky ]`) and emits a notice to `pnpm install` if absent rather than leaning on a parent; runs the prepare script directly (`node scripts/prepare.mjs`, not `pnpm run prepare` — directness over an extra package-manager process); verifies `.husky/_/pre-commit` materialized. It is wired to CwdChanged as well as SessionStart so that moving into a different checkout mid-session (e.g. a `cd` into a sibling worktree) re-runs the same per-environment check. **WorktreeCreate is deliberately NOT wired** — that event replaces git's default worktree-creation behavior (the hook must perform the creation and print the new worktree's absolute path, or creation fails), so a bootstrap-only hook there would break `claude --worktree`; a new worktree is instead bootstrapped by the SessionStart that fires when a session opens in it.

- **`fallow-gate.sh` (PreToolUse Bash)** — gate Claude's `git commit` / `git push` invocations on `fallow audit` verdict. Resolves `fallow` via PATH and `pnpm exec` (the pnpm analogue of `npx --no-install` — runs an already-installed workspace binary, never fetches). Originated the bootstrap-readiness wait (poll for `.husky/_/pre-commit`, watch the lock, break early on lock-disappear) that the agent-facing hooks adopted in incident 10 — but, as a commit safety gate, it `exit 2`-blocks on timeout where the quality hooks fall through.

- **`prettier-format.sh` (PostToolUse Edit|Write|MultiEdit)** — format Claude's edits in place. Scope-check against `CLAUDE_PROJECT_DIR` with `cygpath -u` normalization; rely on `.prettierignore` for the worktree-subpath case the prefix check can't catch. If the local prettier (`$ABS_PROJECT/node_modules/.bin/prettier`) is missing it **waits out any in-flight background bootstrap** before falling through to its resolution chain (incident 10) — otherwise an edit during the install window would silently skip formatting (the invisible-skip failure mode this ADR most wants to avoid). The wait anchors git at `$ABS_PROJECT` (the edited file's project root), since this hook never `cd`s.

- **`eslint-stop.sh` (Stop)** — run `eslint .` before Claude ends a turn; block (`decision: "block"`) on lint errors (exit 1). Uses the LOCAL `./node_modules/eslint/bin/eslint.js` (no upward walk); if that's absent, **first waits out any in-flight background bootstrap** (poll for `.husky/_/pre-commit` up to `HUSKY_GATE_WAIT_SECS`, break early if the lock disappears — incident 10), then blocks with a message telling the agent to `pnpm install` (the `stop_hook_active` guard prevents an infinite loop). Surfaces exit 2 (config error / no files matched / all ignored) loudly to stderr instead of treating it as a silent clean pass. Excludes `.claude/worktrees/` via the shared `eslint.config.mjs` ignores list (and a redundant inline `--ignore-pattern` for runs from the main checkout).

- **`stylelint-stop.sh` (Stop)** — mirrors `eslint-stop.sh` for CSS: runs `stylelint "**/*.css"` and blocks on lint problems (stylelint's exit 2). Same LOCAL-binary requirement (`./node_modules/stylelint/bin/stylelint.mjs`) and the same incident-10 bootstrap wait before its missing-tool block.

- **`.husky/pre-commit`** — runs `pnpm exec lint-staged`, which prettifies + eslint-fixes staged files. Fires in whichever working tree `git commit` was invoked from.

- **`.husky/pre-push`** — mirrors the seven CI gates (`lint`, `format:check`, `typecheck`, `license-check`, `pnpm audit --prod --audit-level high` blocking + full-tree advisory, `audit:fallow`, `build`, `test-storybook`).

## Chronology of failures

Over a dozen commits across ten chronology entries, in commit order. Entries 1–8 are failures (a merged-broken PR, silently-skipped gates, a reformatted-wrong-file, a self-defeating pair); entry 9 is the husky-CI-pattern adoption; entry 10 is the background-install race the async-bootstrap change opened in the agent-facing hooks. Several entries introduced or re-exposed a different problem — incident 8 reverts parts of incident 7, and incident 10 is a regression opened by the async-install optimization that landed after this ADR's original writing.
...
### Incident 10 — async bootstrap raced the agent-facing hooks (2026-05-30, commits [`217ef8c`](https://github.com/bkshrader/notebook-app/commit/217ef8c), [`ad42fc2`](https://github.com/bkshrader/notebook-app/commit/ad42fc2), [`86cec09`](https://github.com/bkshrader/notebook-app/commit/86cec09))

After incident 8 settled the per-environment model, a performance change moved the bootstrap's `pnpm install` into a **detached background process** (`217ef8c`): a cold install on a fresh worktree is ~40s (Windows hard-link cost) and a SessionStart hook blocks the input prompt for its whole duration, so the install was detached to make the session usable in ~0.5s. The readiness cost — for the ~40s install window a fresh worktree has no `node_modules` and no `.husky/_/` — was handled in `fallow-gate.sh` by adding a **bootstrap-readiness wait**: poll for `.husky/_/pre-commit`, watch the `husky-bootstrap.lock`, wait up to `HUSKY_GATE_WAIT_SECS` (120s), break early if the lock disappears, then `exit 2`-block so a commit never skips the gate (`ad42fc2` anchored the check to the worktree root; `86cec09` hardened the stale-lock handling).

That wait was added to the **commit** path only. The three **agent-facing** hooks — `eslint-stop.sh`, `stylelint-stop.sh`, `prettier-format.sh` — each do a LOCAL tool-existence check (invariant 2) that now fires _during_ the install window, before the deps it's checking for have finished installing:

- **`eslint-stop.sh`** / **`stylelint-stop.sh`** — saw the missing binary and immediately returned `decision: "block"` ("no local eslint/stylelint install … run `pnpm install`"). On a fresh worktree both Stop hooks block back-to-back, even though the install is in flight and finishes within the window. Surfaced live: a session opened in a just-created worktree, answered a quick question, and hit the eslint block at Stop time while `node_modules` was materializing in the background.
- **`prettier-format.sh`** — worse, because it fails _invisibly_: with the local prettier absent it fell through its resolution chain (`pnpm exec` → global → "not found, skipping") and silently skipped formatting — the no-silent-skip driver's exact anti-goal.

The defect is that invariant 2's "tool missing ⇒ make noise" predates the async bootstrap (this ADR was written 2026-05-28; `217ef8c` landed later), so "missing right now" no longer implies "never provisioned." Fix: propagate the same readiness wait `fallow-gate.sh` already had to the three agent-facing hooks, gating each one's missing-tool branch behind a bounded poll for `.husky/_/pre-commit` (anchored to the worktree root via `git rev-parse --show-toplevel`, lock via `git -C "$ROOT" rev-parse --git-path`, early-exit on lock-disappear). The key difference from `fallow-gate.sh`: these are **quality nudges, not safety gates**, so on timeout they **fall through to their existing behavior** (Stop hooks block-once; prettier silently skips) rather than `exit 2`. In practice the ~40s install finishes well inside the 120s wait, so the tool is present and the hook just runs normally.

Implementation note — **inline copy, not a shared helper.** The wait block was duplicated into each of the three hooks (now four copies including `fallow-gate.sh`) rather than extracted into a sourced `lib/`. This deliberately preserves the property noted throughout this ADR that each hook is self-contained with zero cross-file `source` dependencies (and keeps invariant 3's per-environment isolation trivially obvious — no shared file to mis-resolve across worktrees). The tradeoff is accepted maintenance duplication: the four copies must be kept in sync, which is the same duplication that let this gap exist in the first place (fallow-gate had the wait; the others didn't). If a fifth consumer appears, revisit extracting a sourced helper.
