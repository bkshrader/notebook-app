...
- **`fallow-gate.sh` (PreToolUse Bash)** — gate Claude's `git commit` / `git push` invocations on `fallow audit` verdict. Resolves `fallow` via PATH and `pnpm exec` (the pnpm analogue of `npx --no-install` — runs an already-installed workspace binary, never fetches).
