...
4. **`.husky/_/` must exist in every environment that might receive a commit or push.** Git treats a missing `core.hooksPath` target as "no hook" and silently no-ops. Husky's installer only writes `.husky/_/` to whichever working tree it's run from. A worktree-bound session that commits via `git -C <main repo>` hits the main repo's git dir; if its `.husky/_/` is missing the commit silently bypasses all hooks. The SessionStart bootstrap (`.claude/hooks/session-start-bootstrap.sh`) must materialize `.husky/_/` in _both_ the worktree and the main repo on every session entry (idempotent, ~37ms when already bootstrapped).
...
- **Investigate what's deleting `.husky/_/` from the main repo.** Incident 7's root cause was the missing wrapper; the bootstrap fix repaired it but didn't identify what removed it in the first place. Candidate causes: fallow-audit cache invalidation, the worktree-cleanup flow when a worktree's `_/` was a symlink (it isn't, but the cleanup may not check), a `git clean -fdx` that included `.husky/_/` because it's in `.gitignore`. Until this is known, the bootstrap is treating the symptom, not the cause.
...
- **Evaluate whether the worktree-cleanup flow should _not_ leave half-empty worktree directories.** The `lucid-jennings` worktree this session was bound to was an empty directory with a valid git dir — that's a fragile state that contributed to incident 7. Either fully remove the worktree on cleanup or leave it intact.
...
- [husky `index.js`](https://github.com/typicode/husky/blob/main/index.js) — confirms `.husky/_/` is load-bearing; `core.hooksPath = .husky/` doesn't work because the `_/` wrapper is what puts `node_modules/.bin` on PATH and honors `HUSKY=0`.
