...
- **No code-display / code-editing primitive.** Pair Ark with CodeMirror 6 (MIT) for the Code Block (read-only) and Code Editor surfaces — the same plain-DOM-host split as the table, mounting a `.cm-*` editor under a `[data-scope][data-part]` host. CM6 is the project's decided editor substrate (`docs/research/codemirror.md`); its contenteditable rendering is the accessible choice over Prism/Monaco.
