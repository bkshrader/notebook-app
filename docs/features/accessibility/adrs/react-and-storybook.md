...
- Storybook adds `@storybook/addon-mcp` and `@chromatic-com/storybook` by default on `init`. `@chromatic-com/storybook` was removed (no Chromatic SaaS plan) — this was a default-pruning, not a considered rejection of visual-regression testing; that decision is deferred-not-rejected, with the revisit triggers recorded in [storybook-testing.md §5](../../../research/storybook-testing.md#5-recommendation-for-notebook-app). `@storybook/addon-mcp` was initially removed as out-of-scope but reinstated immediately after, per [Storybook's MCP setup docs](https://storybook.js.org/docs/ai/setup): it exposes the Storybook manifest, story-generation, and interaction-test surface to AI coding agents as an MCP server at `http://localhost:6006/mcp`. The server is registered for Claude Code at project scope in [`.mcp.json`](../../../../.mcp.json), and [`CLAUDE.md`](../../../../CLAUDE.md) directs agents to consult it before doing UI work. If `@chromatic-com/storybook` reappears after a future `storybook upgrade`, remove again and note here.
